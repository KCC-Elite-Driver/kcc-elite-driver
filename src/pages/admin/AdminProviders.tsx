...
const AdminProviders = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Supprimer ce prestataire et tous ses chauffeurs ?")) return;
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card">
        <div className="container mx-auto flex items-center justify-between h-16 px-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-serif text-xl font-bold text-foreground">Prestataires</h1>
          </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-serif text-lg font-semibold text-foreground">{providers.length} prestataire{providers.length !== 1 ? "s" : ""}</h2>
        ...
        {showForm && (
          <div className="rounded-lg border border-border bg-card p-5 mb-6 space-y-4">
            <h3 className="font-serif text-base font-semibold text-foreground">{editId ? "Modifier" : "Nouveau"} prestataire</h3>
        ...
        <div className="space-y-3">
          ...
          {providers.length === 0 && !showForm && (
            <p className="font-sans text-sm text-muted-foreground text-center py-8">Aucun prestataire. Ajoutez-en un pour commencer.</p>
          )}
        </div>
      </main>
    </div>
