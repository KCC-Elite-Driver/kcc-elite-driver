    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4 pt-20">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="font-serif text-2xl font-bold text-foreground">{t.client_login_title}</h1>
          <p className="font-sans text-sm text-muted-foreground mt-1">{t.client_login_desc}</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? t.auth_login_loading : t.auth_login}
          </button>
        </form>
        <p className="font-sans text-sm text-muted-foreground text-center mt-6">
          {t.auth_no_account}{" "}
          <Link to="/client/register" className="text-primary hover:underline">{t.auth_signup}</Link>
        </p>
      </div>
    </div>
  );
