...
type Booking = {
  ...
  date: string; time: string; vehicle: string | null; status: string; created_at: string;
  firstname: string; lastname: string; email: string; phone: string;
  passengers: number; luggage: number; notes: string | null;
  flight_number: string | null; meet_greet: boolean; payment_method: string | null;
};
...
const ClientBookings = () => {
  ...
  const handleRebook = (b: Booking) => {
    const params = new URLSearchParams({
      service: b.service_type || "", pickup: b.pickup, dropoff: b.dropoff,
      vehicle: b.vehicle || "", firstname: b.firstname, lastname: b.lastname,
    ...
    <div className="min-h-screen bg-background pt-20">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-serif text-2xl font-bold text-foreground">{t.client_my_bookings}</h1>
          ...
          <div className="flex items-center gap-3">
            <Link to="/booking" className="flex items-center gap-1.5 gradient-gold text-primary-foreground font-sans text-sm font-semibold px-4 py-2 rounded-md hover:opacity-90">
              <Plus size={16} /> {t.client_new_booking}
            </Link>
        ...
        {bookings.length === 0 ? (
          <div className="text-center py-16">
            <p className="font-sans text-muted-foreground mb-4">{t.client_no_bookings}</p>
            ...
            {bookings.map(b => (
              <div key={b.id} className="rounded-lg border border-border bg-card p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <span className="font-sans text-sm font-medium text-foreground">{b.service_type || t.admin_bookings}</span>
                    <span className="font-sans text-xs text-muted-foreground ml-3">{b.date} {b.time}</span>
                  </div>
                ...
                <div className="flex gap-2">
                  <Link to={`/client/bookings/${b.id}`}
                    ...
                    <Eye size={12} /> {t.client_details}
                  </Link>
                  {b.status === "completed" && (
                    <button onClick={() => handleRebook(b)}
                      className="flex items-center gap-1 font-sans text-xs text-primary hover:underline">
                      <RotateCcw size={12} /> {t.client_rebook}
                    </button>
                  )}
                </div>
              </div>
